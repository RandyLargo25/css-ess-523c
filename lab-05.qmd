...
```{r}
#neural network (bagged MLP via baguetta)
nn_model <- bag_mlp() %>%
  set_engine("nnet") %>%
  set_mode("regression")
```
...
My written answer: Among the four models, the neural network performed the best, achieving the highest R\^2 and lowest RMSE under cross validation. This indicates it is most effective at capturing the nonlinear relationships between aridity, precip, and streamflow. I would move forward with the neural network model because it provides that strongest predictive, while the weaker performance of XGBoost likely reflects the lack of hyperparameter tuning.
