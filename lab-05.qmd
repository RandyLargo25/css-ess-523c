...
My written answer: zero_q_freq represents the proportion of time that streamflow at a given gauge is equal to zero. it measures how frequently a river basin experiences no-flow conditions over the observation period. Basins with higher values of zero_q_freq are more likley to be intermittent or ephemeral stream, while values near zero indicate perennial streams that flow year round.
...
My written answer: The Map shows a clear spatial pattern in the mean stream flow: q_mean across the US. Higher flows: blue are concentrated in the wetter regions such as the Pacific Northwest and the eastern US, while lower flow: pink apper in arid regions like the southwest. This pattern reflects underlying climate gradients, such as precipitation and evapotranspiration, which vary smoothly across space. The strong geographic structure suggests that location plays an important role in determining streamflow, meaning that some predictors in later models may act as proxies for spatial position rather than purely physical processes.
...
```{r}
#checking the if data is in camels
glimpse(camels)
```
...
borders("state", colour = "gray50") +
  ...
  ggthemes::theme_map() +
  labs(
    title = "Spatial Distribution of Aridity",
    x = "Longitudue",
    y = "Latitude",
    color = "Aridity"
  )
...
map_precip <- ggplot(camels, aes(x = gauge_lon, y = gauge_lat)) + 
  ...
  ggthemes::theme_map() +
  labs(
    title = "Spatial Distribution of Mean Precipitatoin", 
    x = "Longitude",
    y = "Latitude",
    color = "Precipitation"
  )
...
My written answer: The map show clear spatial patterns in both aridity and precipitation across the US. Aridity is highest in the western and southwestern regions, indicating drier climates, while lower aridity values are found in the eastern US in contrast, mean precipitation (p_mean) is highest in the Pacific Northwest and eastern regions, and lowest in the interior West and Southwest. These patterns are stongly inversely related, as expected, with wetter regions exhibiting lower aribilty and regions exhibiting higher aribity. This spatial structure suggests that both variables are closely tied to geogrpahic and climatic gradients, which will likely influence model predictions of streamflow.
...
```{r}
#neural network (bagged MLP via baguette)
nn_model <- bag_mlp() %>%
  set_engine("nnet") %>%
  set_mode("regression")
```
...
#cls_metrics <- metric_set(rmse, rsq, mae) #added in later

wf <- wf %>%
  workflow_map(
    "fit_resamples",
    resamples = camels_cv
    #metrics = cls_metrics. #added in later
  )
```
...
My written answer: Among the four models, the neural network got a higher R\^2 and lowest RMSE among the other models and indicateing the strongest predictive performance. This suggests it is better able to capture the nonlinear relationships between aridity, precipitation, and streamflow compared to the liner model and tree-based approaches. Going forward with the neural network model maybe ok because of its good performance, through the relatively similar performance of the random forest and XGBoost models suggest that additional hyperparmater tuning could further improve those models.
...
```{r}
#using skimr and or visdat
library(skimr)
library(visdat)

skim(camels)    #for info
vis_dat(camels)
```

My written answer: The data-set contains 671 observation and 59 values, with the majority (53) being numeric predictors and a small number categorical variables. Missingness is generally low, with most variables having complete data, through a few variables (geo1_2nd_class, root_depth\_\*) contain moderate missing values. May hydrologic and environmental variables, including q_mean, p_mean, and basin area metrics, exhibit strong right- skewed distributions, as indicated by their histograms and summary statistics. These patterns suggest the need for preprocessing steps such as log. transformation for skewed variables, normalization for differing scales, and handling missing values within the modeling recipe.
...
ggplot(camels, aes(x = logQmean)) +
  ...
  theme_linedraw() +
  labs(title = "Distribution of Log Mean Streamflow")
```
...
ggplot(camels, aes(x = p_mean)) +
  ...
  theme_linedraw() +
  labs(title =  "Distribution of Mean Precipitation")
```
...
My written answer: The distribution of q_mean is highly right skewed, with many basins have low steamflow and a small number exhibition very high values. Applying a log transformation relsuts in a more symmetric distribution, supporting it use as the modeling outcome. Similar right skewed patterns are observed in predictors such as precipitation and basin area, indicating that transformations may also be beneficial for predictors.
...
```{r}#
#issue samp and logQmean not found, may have deleted funcion?
set.seed(10)
samp_clean <- samp |> drop_na()   #samp missing?

coords <- cbind(samp_clean$gauge_lon, samp_clean$gauge_lat)
...
moran_results <- samp_clean |>
  select(where(is.numeric), -q_mean, -logQmean) |>    #logQmean missing?
  map_dfr(
...
```{r}
#to fix error
# Create log-transformed outcome FIRST
camels <- camels |> 
  mutate(logQmean = log(q_mean))
...
My written answer: Morgan's I results show strong positive spatial autocorrelation in several predictors. As expected, geographic coordinates (gauge_1on and gauge_1at) exhibit the highest values, confirming strong spatial structure. Importantly, climate variables such as precipitation, precipitation seasonality, and vegetation indices also show high Morgan's I values, indicating that they vary smoothly across space. This suggests that many predictors carry geographic info and may act as proxies for locations. As a result, models trained on these features may partially rely on spatial patterns rather than purely physical relationships, reinforcing the need for spatial cross-validation to properly assess generalization.
...
My written answer: The data were split into 75% training and 25% testing set to ensure sufficient data for model training while retaining an independent data-set for evaluation. Stratification was not add because the outcome variable is continuous, and it is distribution, while right skewed, does not show strong discrete grouping that would require stratified sampling. Additionally, spatial structure is a key concern in this dataset, this will be addressed better by using spatial cross validation rather than stratified random splits.
...
I choose: Using formula for to predict (logQmean)
logQmean ~ aridity + p_mean + pet_mean + soil_porosity + elev_mean + frac_forest

My written answer: The model is designed to predict log transformed mean stream-flow (logQmean) using climatic, soil, vegetation, and topographic catchment attributes. A log transformation is applied to the response variable to reduce right skewness observed in the distribution of q_mean and to stabilize variance across basins. Key predictors such as aridity, mean precipitation (p_mean) and potential evapotranspiration (pet_mean) are included because EDA and correlation analysis shhow strong relationships with stream flow. Soil and vegetation variables are included because they represent catchment storage and infiltration capacity, which influence runoff generation. Topographic variables such as elev_mean are included because elevation controls temp., Precip phase, and runoff timing. Several predictors exhibit skewed distributions, so normalization is applied to ensure comparable scales for models sensitive to magnitude differences. Missing values are removed to make sure the model is stabel and compatibility across re-sampling folds.

```{r}
#for recipe 
rec <- recipe(logQmean ~ aridity + p_mean + pet_mean +
                soil_porosity + elev_mean + frac_forest,
              data = camels_train) %>%
  ...
  # normalize predictors for modeling stability
...
```{r}
#choseing random forest
rf_model <- rand_forest() %>%
  set_engine("ranger") %>%
  set_mode("regression")
```

My wirtten answer: The random forest works for this problem because they capture nonlinear interactions between climate and catchment attributes (precip, aridity, soil controls on runoff) without requiring explicit functional assumptions, which is important given the heterogeneous and spatially varying hydrologic regimes in CAMELS.
...
My written answer: XGBoost is good for this data set because it models complex nonlinear structure through sequential boosting, allowing it to iteratively correct errors in stream flow predictions driven by interacting hydrologic controls

```{r}
#for neural network (bag_mlp)
...
My written answer: a neural network is a suitable because it can approximate highly nonlinear functional relationship between climate, soil, and vegetation variables and stream-flow through layered feature transformation.
...
```{r}
#original
#convert to spatial object (for spatial CV)
...
```{r}#
#original code
#spatial CV, choosing spatial block CV
set.seed(123)

camels_spatial_cv <- spatial_block_cv(
  camels_sf,
  v = 10
)
```
...
```{r}
#testing 
camels_spatial_cv <- spatial_clustering_cv(
  camels_sf,
  v = 10
)
```
...
wf <- workflow_set(
  ...
  models = list(
    linear = lm_model,
    rf = rf_model,
    xgb = xgb_model,  #changed xgd to xgb
    nn = nn_model
  )
)
```
...
autoplot(wf_random)
rank_results(wf_random, rank_metric = "rsq")

autoplot(wf_spatial)
rank_results(wf_spatial, rank_metric = "rsq")
```
...
wf_random %>% collect_metrics() %>% mutate(type = "random") %>%
  bind_rows(
    wf_spatial %>% collect_metrics() %>% mutate(type = "spatial")
  ) %>%
  ggplot(aes(x = wflow_id, y = mean, fill = type)) +
  geom_col(position = "dodge") +
  facet_wrap(~ .metric)
```

My written answer: Under random cross-validation, the models show strong predicitve performance, with the random forest and XGBoost models typically achieving the heighest R\^2 values. However, performance decreases under spatial cross validation, indicating that some of predictive power in the random CV case was inflated by spatial autocorrelation between nearby basins. The random forest model generally remains the most stable across both re-sampling strategies, suggesting it capiutres more transferable hydrologic structure rather than purely gerographic proximity. The drop in performance form random to spatial CV represents a more realistic estimate of generalization error and should be preferred for interpreting real world predictive skill. The spread in fold level metrics under spatial CV suggests that model performance varies across climatic and physiographic regions, with some basins being systematically harder to predict than others. Overall, the spatial CV results provide a more honest assessment of model generalization and indicate that geographic structure plays a meaningful role in stream flow prediction.
...
#issue down stream
#Tune Random Forest
#defining model with tunable hyperparameters
rf_tune <- rand_forest(
  mtry = tune(),
  min_n = tune(),
  trees = 500
...
```{r}
#added "importance =" for fix
rf_tune <- rand_forest(
  mtry = tune(),
  min_n = tune(),
  trees = 500
) %>%
  set_engine("ranger", importance = "impurity") %>%   # ← THIS IS THE FIX
  set_mode("regression")
```


```{r}
#building a name grid
...
My written answer: a regular grid was used to systematically explore combinations of tree complexity and smoothing (min_n), ensuring coverage of full parameter space rather than random sampling.
...
```{r}
#tuning landscape
autoplot(rf_tuned) #takes a while
```
...
final_rf_fit <- finalize_workflow(rf_wf_tune, best_rf) %>%
  fit(data = camels_train)
```

My written answer: The most influential hyperparameter was mtry, indicating that the number of predictors considered at each splot strongly affects predictive performance, reflecting the balance between bias and variance. Smaller min_n values generally imporved pergormance, suggesting that deeper trees better capture heterogeneity in hydrologic responses across the basins. The model shows moderate sensitivity to tree complexity, but performance stablizies once a sufficient level of randomness is introduced. 
...
Using: best_rf <- select_best(rf_tuned, metric = "rsq")
```{r}
#finalize tuned workflow
final_rf_wf <- finalize_workflow(rf_wf_tune, best_rf)
```

```{r}
#last_fit()  clean evaluation

final_rf_last <- last_fit(
  final_rf_wf,
  split = camels_split
)
```
...
collect_metrics(final_rf_last)
```

```{r}
#for getting predictions

preds <- collect_predictions(final_rf_last)
```
...
```{r}#
# error, rejoin test data manually did not work for aridiy and logQmean
preds <- collect_predictions(final_rf_last) %>%
  left_join(camels_test %>% mutate(.row = row_number()), by = ".row")
```

```{r}#
#for error
test_data <- camels_test %>%
  mutate(.row = row_number())

preds <- bind_cols(test_data, preds)
```

```{r}#
#blocked for render issue
#for error
#found there are two logQmean colmun in data
preds_clean <- preds %>%
  select(logQmean = logQmean...63,
         .pred,
         aridity)
```

```{r}
preds_clean <- collect_predictions(final_rf_last)

preds_clean <- preds_clean %>%
  mutate(
    logQmean = camels_test$logQmean,
    aridity  = camels_test$aridity
  )
```
...
```{r}#
#render issue
ggplot(preds_clean, aes(x = logQmean, y = .pred, color = aridity)) +
  geom_point(alpha = 0.7) +
  geom_abline(linetype = "dashed") +
  scale_color_viridis_c() +
  theme_linedraw() +
  labs(
    title = "Final Random Forest Model: Observed vs Predicted Streamflow",
    x = "Observed log(Qmean)",
    y = "Predicted log(Qmean)",
    color = "Aridity"
  )
```
```{r}
...
My written answer: The final random forest model achieved an R^2 of approximatel 0.88 on the held out test set with an RMSE of 0.40, showing a strong predictive performance for log mean streamflow. This performance is slightly higher than the spatial cross validation estimate, suggesting that some residual spatial dependence my still be inflating random split performance. The model appears to perform best in moderate aridity regions but shows greater error in extreme dry basins, where streamflow is more intermittent. The results suggest good generalization ability, but spatial structure in the data still plays an important role in prediction accuracy. 
